...
describe('+ copySync()', function () {
  ...
  describe('> when the source is a file', function () {
    ...
    describe('> when the src and the dest are identical', function () {
      var file
      var fileData = 'some data'
      var src, dest

      beforeEach(function() {
        file = path.join(TEST_DIR, 'TEST_fs-extra_file')
        fs.writeFileSync(file, fileData)
      })

      describe('> when src and dest are the same file', function () {
        it('should not copy and not throw an error', function () {
          ...
          assert.strictEqual(fileData, destData)
        })
      })

      describe('> when src is symlink and points to dest', function () {
        it('should not copy and keep the symlink', function () {
          src = path.join(TEST_DIR, 'TEST_fs-extra_file_link')
          dest = file
          fs.symlinkSync(file, src)

          fs.copySync(src, dest)
          var destData = fs.readFileSync(dest, 'utf8')
          assert.strictEqual(fileData, destData)
          assert.strictEqual(dest, fs.readlinkSync(src))
        })
      })

      describe('> when dest is symlink and points to src', function () {
        it('should not copy and keep the symlink', function () {
          src = file
          dest = path.join(TEST_DIR, 'TEST_fs-extra_file_link')
          ...
          var destData = fs.readFileSync(dest, 'utf8')
          assert.strictEqual(fileData, destData)
          assert.strictEqual(src, fs.readlinkSync(dest))
        })
      })
    })
